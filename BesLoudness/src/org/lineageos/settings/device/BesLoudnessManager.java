...
public class BesLoudnessManager {
    ...
    public static boolean get(final Context context) {
        final SharedPreferences prefs = context.getSharedPreferences(SHARED_PREFS_NAME, 0);
        boolean exceptedValue = prefs.getBoolean(KEY_BESLOUDNESS, DEFAULT_VALUE);
        final AudioManager amgr = (AudioManager) context.getSystemService("audio");
        boolean actualValue = !("GetBesLoudnessStatus=0".equals(amgr.getParameters("GetBesLoudnessStatus")));
        if (actualValue != exceptedValue) {
            Log.e(TAG, "value mismatch, excepted " + exceptedValue + ", got " + actualValue);
        }
        return actualValue;
    }
}

...
AGnss::AGnss(const AGpsInterface_ext* aGpsIface) : mAGnssIface(aGpsIface) {
...
void AGnss::statusCb(AGpsStatus* status) {
    ...
    auto ret = sAGnssCbIface->agnssStatusCb(
            static_cast<IAGnssCallback::AGnssType>(status->type),
...
/*
 * Implementation of methods from ::android::hardware::gnss::V2_0::IAGnss follow.
 */
...
Return<bool> AGnss::dataConnOpen(uint64_t networkHandle, const hidl_string& apn,
    ...
    return (mAGnssIface->data_conn_open_with_apn_ip_type(
            networkHandle, apn.c_str(), static_cast<uint16_t>(apnIpType)) == 0);
}

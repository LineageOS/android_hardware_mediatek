/*
 * Driver interaction with MediaTek extended Linux CFG80211
 *
...
/**
 * mediatek_get_sta_statistics - Get and format station statistics
 * @wpa_s: wpa_supplicant data
 * @addr: Station MAC address
 * @buf: Buffer to store formatted output
 * @buflen: Length of buffer
 *
 * Returns: Length of formatted data or negative on error
 */
static int mediatek_get_sta_statistics(struct wpa_supplicant *wpa_s, char *addr,
                                      char *buf, size_t buflen)
{
    ...
    os_memset(&sta_statistics, 0, sizeof(sta_statistics));

    if (hwaddr_aton(addr, sta_addr)) {
        wpa_printf(MSG_ERROR, "MediaTek: Invalid station address '%s'", addr);
        return -1;
    }

    /* Try to get the correct MAC for P2P stations */
    mac = mediatek_get_p2p_addr(wpa_s, sta_addr);

    if (mediatek_get_station_stats(wpa_s, mac ? mac : sta_addr, (u8 *)&sta_statistics) < 0) {
        wpa_printf(MSG_ERROR, "MediaTek: Failed to get station statistics");
        return -1;
    }

    len = mediatek_format_station_stats(wpa_s, &sta_statistics, buf, buflen);
    mediatek_log_station_stats(&sta_statistics);
...
/**
 * mediatek_handle_pmkid_cmd - Handle PMKID update command
 * @priv: Private driver data
 * @buf: Command buffer
 *
 * Returns: 0 on success, negative on failure
 */
static int mediatek_handle_pmkid_cmd(void *priv, char *buf)
{
    ...
    wpa_printf(MSG_INFO, "MediaTek: Updating PMKID - BSSID: " MACSTR ", STA: " MACSTR
             ", Action: %s", MAC2STR(entry->bssid), MAC2STR(entry->sta),
             entry->addremove ? "Add" : "Remove");
...
static int mediatek_process_driver_cmd(struct driver_cmd_context *ctx, int *handled)
{
    ...
    *handled = 1;
    ...
    else if (os_strncasecmp(ctx->cmd, "GET_STA_STATISTICS ", 19) == 0) {
        if (ctx->wpa_s) {
            ret = mediatek_get_sta_statistics(ctx->wpa_s, ctx->cmd + 19, ctx->buf, ctx->buf_len);
            if (ret < 0)
                ret = -1;
        }
    } 
    ...
    else if (os_strncasecmp(ctx->cmd, "UPDATE_STA_PMKID", os_strlen("UPDATE_STA_PMKID")) == 0) {
        ret = mediatek_handle_pmkid_cmd(ctx->bss, ctx->buf);
    }
